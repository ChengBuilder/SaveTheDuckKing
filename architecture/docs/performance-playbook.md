...
重点字段：
1. `targetFps`：当前帧率策略最终命中的目标值
2. `pixelRatio`：当前实际读取到的运行时像素比
3. `platformStrategy.startupMode`：平台启动是 `immediate` 还是 `next-frame`
4. `metrics.performance.renderPolicy.targetRenderDpr`：最终渲染倍率
5. `metrics.performance.renderPolicy.targetRenderWidth / targetRenderHeight`：目标渲染尺寸
