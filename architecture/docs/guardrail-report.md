...
## 总览
- 最近执行时间：2026-04-08T03:50:53.101Z
- 项目目录：`./`
- 已执行步骤：20
- 通过步骤：20
- 失败步骤：0
...
| 步骤 | 状态 | 耗时（ms） | 命令 |
| --- | --- | ---: | --- |
| 主入口语法检查 | 通过 | 93 | `node -c game.js` |
| 启动编排语法检查 | 通过 | 50 | `node -c architecture/boot/game-bootstrap.js` |
| 启动配置语法检查 | 通过 | 54 | `node -c architecture/boot/config.js` |
...
| 素材分层检查 | 通过 | 83 | `node architecture/tools/check-asset-code-separation.js` |
| 运行安全校验 | 通过 | 64 | `node architecture/tools/verify-runtime-safety.js` |
